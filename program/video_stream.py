...
async def play_tg_file(c: Client, m: Message, replied: Message = None, link: str = None):
    ...
    if replied.video or replied.document:
        if not link:
            ...
            loser = await m.reply("📥 **جاري تحميل الفيديو...**")
        ...
        if chat_id in QUEUE:
            ...
            await m.reply_photo(
                ...
                caption=f"💡 **تمت إضافة المسار إلى قائمة الانتظار »** `{pos}`\n\n"
                        f"🏷 **الاسم:** [{songname}]({url})\n"
                        f"⏱️ **المده:** `{duration}`\n"
                        f"💭 **المجموعه:** `{chat_id}`\n"
                        f"🎧 **طلب بواسطة:** {requester}", 
            )
            ...
            try:
                ...
                await m.reply_text("❌ قم بفتح الكول في الجروب اولا\nاو قم بكتابه `/startvc` !")
            except Exception as e:
                LOGS.info(f"[ERROR]: {e}")
    else:
        await m.reply_text(
...
async def video_stream(c: Client, m: Message):
    ...
    try:
        ...
        if b.status == "banned":
            try:
                await m.reply_text("❌ قام احد المشرفين بحظر الحساب المساعد\nقم بفك حظره ثم اعد المحاوله !")
                await remove_active_chat(chat_id)
            except BaseException:
                pass
    ...
    if replied:
        ...
        if len(m.command) < 2:
            await m.reply_text("» الرد على ** ملف فيديو ** أو ** أعط شيئًا للبحث **")
